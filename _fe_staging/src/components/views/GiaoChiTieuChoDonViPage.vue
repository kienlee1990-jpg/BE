            ...
            <div class="container-fluid py-4">
                ...
                <div v-if="showModal" class="modal fade show d-block custom-modal" tabindex="-1"
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content border-0 shadow-lg rounded-4">
                            ...
                            <div class="modal-body pt-3">
                                <div class="row g-3">
                                    ...
                                    <div v-if="!isDinhLuongSoSanhCriterion(form.tieuChiDanhGia || selectedDanhMuc?.loaiChiTieu)" class="col-12 col-md-4">
                        ...
                        <option v-for="item in getQuyTacOptionsByCriterion(form.tieuChiDanhGia || selectedDanhMuc?.loaiChiTieu, form.kieuSoSanh)" :key="item.value" :value="item.value">
                                                {{ item.label }}
                                            </option>
                                        </select>
                                    </div>
                            ...
                            <div v-if="isDinhLuongSoSanhCriterion(form.tieuChiDanhGia) && isChenhLechComparisonKind(form.kieuSoSanh)" class="col-12 col-md-6">
                                            ...
                                            <div v-else class="row g-3">
                                                <div class="col-12">
                                                    ...
                                                    <textarea v-model="form.giaTriMucTieuText" rows="2"
                                                        class="form-control"
                                                        placeholder="Nhập mô tả chỉ tiêu giao để hiển thị trên báo cáo"></textarea>
                                                </div>
                                                <div class="col-12 col-md-4">
                                <label class="form-label">{{ getTargetValueLabel(form.tieuChiDanhGia || selectedDanhMuc.loaiChiTieu) }} <span
                                        class="text-danger">*</span></label>
                                <input v-model="form.giaTriMucTieu" type="number" step="any"
                                    class="form-control" :placeholder="getTargetValuePlaceholder(form.tieuChiDanhGia || selectedDanhMuc.loaiChiTieu)" />
                                                </div>
                                                <div class="col-12 col-md-4">
                                            ...
                                            <div v-for="(child, index) in form.tieuChiCon" :key="child.localKey"
                                                ...
                                                <div
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="small text-muted">{{ mapLoai(child.tieuChiDanhGia || child.loaiChiTieu) }}<template v-if="!isQualitative(child.tieuChiDanhGia || child.loaiChiTieu) && !isDinhLuongSoSanhCriterion(child.tieuChiDanhGia || child.loaiChiTieu)"> • {{ getQuyTacDanhGiaLabel(child.quyTacDanhGia) }}</template> |
                                                            {{ child.donViTinh || '-' }}</div>
                                                    </div>
                                                ...
                                                <div class="row g-3 mb-3">
                                                    <div class="col-12 col-md-4">
                                                        ...
                                                        <div v-if="!isDinhLuongSoSanhCriterion(child.tieuChiDanhGia || child.loaiChiTieu)" class="col-12 col-md-4">
                                                            ...
                                                            <select v-model="child.quyTacDanhGia" class="form-select" :disabled="isQualitative(child.tieuChiDanhGia || child.loaiChiTieu)">
                                                                <option value="">Chọn quy tắc đánh giá</option>
                                            <option v-for="item in getQuyTacOptionsByCriterion(child.tieuChiDanhGia || child.loaiChiTieu, child.kieuSoSanh)" :key="item.value" :value="item.value">
                                                                    {{ item.label }}
                                                                </option>
                                                            </select>
                                                        </div>
                                    ...
                                    <label class="form-label">{{ getTargetValueLabel(child.tieuChiDanhGia || child.loaiChiTieu) }} <span
                                            class="text-danger">*</span></label>
                                    <input v-model="child.giaTriMucTieu" type="number" step="any"
                                        class="form-control" :placeholder="getTargetValuePlaceholder(child.tieuChiDanhGia || child.loaiChiTieu)" />
                                                    </div>
...
import ColumnVisibilityTools from '../shared/ColumnVisibilityTools.vue'
    ...
    const getEffectiveKieuSoSanh = (item) => {
    ...
    watch(() => form.kieuSoSanh, () => {
        if (syncingForm.value) return
        ensureCriterionDefaults(form)
    })

    watch(
        () => form.tieuChiCon.map(child => `${child.tieuChiDanhGia || ''}|${child.kieuSoSanh || ''}`),
        () => {
            if (syncingForm.value) return
    ...
    const buildPayloadForDonVi = (donViId) => ({
        ...
        giaTriDauKyCoDinh: selectedDanhMuc.value && !selectedDanhMuc.value.tieuChiDanhGias.length && !isQualitative(form.tieuChiDanhGia || selectedDanhMuc.value.loaiChiTieu)
            ...
            : null,
        giaTriMucTieuText: selectedDanhMuc.value && !selectedDanhMuc.value.tieuChiDanhGias.length
    ...
    const validateCriterionConfig = (item, label) => {
        ...
        if (isDinhLuongSoSanhCriterion(item.tieuChiDanhGia) && !item?.kieuSoSanh) {
            alert(`Vui lòng chọn kiểu so sánh cho ${label}.`)
            return false
        }
        ...
        if (isDinhLuongSoSanhCriterion(item.tieuChiDanhGia) && isChenhLechComparisonKind(item.kieuSoSanh)) {
