        ...
        <div class="content-shell">
            <header class="topbar">
                ...
                <div class="topbar-right">
                    <button type="button" class="font-toggle-btn" @click="toggleFontTheme">
                        <i class="bi bi-type"></i>
                        <span>{{ fontThemeLabel }}</span>
                    </button>
...
<script setup>
    ...
    const FONT_THEME_KEY = 'ui_font_theme'
    const dropdownOpen = ref(false)
    const isCollapsed = ref(false)
    const fontTheme = ref('default')
    ...
    const fontThemeLabel = computed(() =>
        fontTheme.value === 'elegant' ? 'Font trang trọng' : 'Font chuẩn'
    )

    const applyFontTheme = () => {
        document.body.setAttribute('data-font-theme', fontTheme.value)
    }

    const toggleFontTheme = () => {
        fontTheme.value = fontTheme.value === 'elegant' ? 'default' : 'elegant'
        localStorage.setItem(FONT_THEME_KEY, fontTheme.value)
        applyFontTheme()
    }
    ...
    onMounted(async () => {
        fontTheme.value = localStorage.getItem(FONT_THEME_KEY) || 'default'
        applyFontTheme()
