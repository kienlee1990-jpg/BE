...
const getHost = () => document.getElementById(`tools-${props.tableId}`)
...
const syncHostWidth = () => {
  ...
  const targetWidth = Math.max(table.scrollWidth, host.parentElement?.clientWidth || 0)
...
onMounted(async () => {
  ...
  window.addEventListener('resize', syncHostWidth)
...
<style>
.table-tools{position:relative;display:flex;justify-content:flex-end;align-items:center;margin-bottom:14px;padding-left:12px}
